...
class MainWidget(RelativeLayout):
    ...
    title_difficulty = StringProperty("Normal")
    ...
    def init_audio(self):
        ...
        self.sfx_point = SoundLoader.load('./audio/bonus_point.mp3')
    ...
    def init_bonus_tiles(self):
        with self.canvas:
            Color(1, 1, 0)
            for i in range(0, self.NB_BONUS_TILES):
                self.bonus_tiles.append(Quad())

    def add_bonus_tile(self, idx):
        with self.canvas:
            Color(1, 1, 0)
            self.bonus_tiles.append(Quad())

    def del_bonus_tile(self, idx):
        self.bonus_tiles_coordinates[idx] = (-100, -100)

        bonus_tile = self.bonus_tiles[idx]
        bonus_tile_coordinates = self.bonus_tiles_coordinates[idx]
        xmin, ymin = self.get_tile_coordinates(bonus_tile_coordinates[0], bonus_tile_coordinates[1])
        xmax, ymax = self.get_tile_coordinates(bonus_tile_coordinates[0] + 1, bonus_tile_coordinates[1] + 1)
        ...
        bonus_tile.points = [x1, y1, x2, y2, x3, y3, x4, y4]
    ...
    def check_plr_on_bonus(self):
        for i in range(0, len(self.bonus_tiles_coordinates)):
            ti_x, ti_y = self.bonus_tiles_coordinates[i]
            if ti_y > self.current_y_loop + 1:
                return False
            if self.check_plr_on_tile(ti_x, ti_y):
                self.del_bonus_tile(i)
                self.sfx_point.play()
                return True
        return False
    ...
    def generate_tile_coordinates(self):
        ...
        for i in range(len(self.bonus_tiles_coordinates) - 1, -1, -1):
            if self.bonus_tiles_coordinates[i][1] < self.current_y_loop:
                del self.bonus_tiles_coordinates[i]
        ...
        for i in range(len(self.tiles_coordinates), self.NB_TILES):
            ...
            if r == 0:
                if (len(self.bonus_tiles_coordinates) < len(self.bonus_tiles) and
                        not last_x + 1 >= end_idx and not last_x - 1 <= start_idx):
                    side = random.randint(0, 1)
                    self.bonus_tiles_coordinates.append((last_x + side * 2 - 1, last_y))
            if r == 1:
                last_x += 1
                if last_x >= end_idx:
                    last_x = end_idx - 1
    ...
    def update_tiles(self):
        ...
        for i in range(0, len(self.bonus_tiles_coordinates)):
            bonus_tile = self.bonus_tiles[i]
            bonus_tile_coordinates = self.bonus_tiles_coordinates[i]
            xmin, ymin = self.get_tile_coordinates(bonus_tile_coordinates[0], bonus_tile_coordinates[1])
            xmax, ymax = self.get_tile_coordinates(bonus_tile_coordinates[0] + 1, bonus_tile_coordinates[1] + 1)
    ...
    def game_speedup(self):
        if self.game_score > 1000000:
            self.game_score = 1000000
        self.game_speed_y += self.SPEED * pow(10, self.game_difficulty * 2 - 10) * \
            self.game_score + self.game_speed_level * pow(10, -10)
        self.game_speed_x += self.SPEED_X * pow(10, self.game_difficulty * 2 - 10) * \
            self.game_score + self.game_speed_level * pow(10, -10)
    ...
    def update(self, dt):
        ...
        if not self.game_over and self.game_start:
            ...
            while self.current_offset_y >= spacing_y:
                ...
                score_display = self.game_score + self.game_score_bonus
                self.title_score = "Score: " + str(score_display)

                print(f'Game Speed:', self.game_speed_y)

                self.game_speedup()
                if self.game_score > 100 and self.game_speed_level == 0:
                    self.game_speed_level += 1
                elif self.game_score > 1000 and self.game_speed_level == 1:
                    self.game_speed_level += 1
                elif self.game_score > 10000 and self.game_speed_level == 2:
                    self.game_speed_level += 1
                elif self.game_score > 100000 and self.game_speed_level == 3:
                    self.game_speed_level += 1
                elif self.game_score > 1000000 and self.game_speed_level == 4:
                    self.game_speed_level += 1

                if self.game_score % self.next_bonus == 0:
                    self.score_bonus *= 10
                    self.next_bonus *= 100
        ...
        if self.check_plr_on_bonus():
            self.game_score_bonus += 10000
    ...
    def difficulty_btn_pressed(self):
        self.game_difficulty += 1
        if self.game_difficulty > 2:
            self.game_difficulty = 0

        match self.game_difficulty:
            case 0:
                self.title_difficulty = "Easy"
            case 1:
                self.title_difficulty = "Normal"
            case 2:
                self.title_difficulty = "Hard"
